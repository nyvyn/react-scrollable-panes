...
export const SlipStackContainer = forwardRef<SlipStackHandle, Props>(
  function SlipStackContainer({ paneData, paneWidth }: Props, ref): ReactNode {
    ...
    // Signed offset generated by the wheel handler
    const offset = extraTabs;                // +n = shifted right,  -n = shifted left

    // 1.  How many left-hand tabs are still shown after applying the offset
    let leftCount = baseLeft - offset;       // reduce when offset is +, increase when –
    leftCount = Math.max(0, Math.min(leftCount, panes.length - 1));

    // 2.  Any remaining positive offset becomes right-hand tabs
    const rightCount = Math.max(
        0,
        Math.min(offset - baseLeft, panes.length - leftCount - 1)
    );

    // 3.  Pinned + track panes that remain visible
